...
wss.on("connection", function (ws) {
	...
	ws.on("message", function (message) {
		...
		if(data.message == "name"){
			if(data.playernr == 1){
				...
				games[data.id].player2.socket.send(JSON.stringify({game: games[data.id], message: "game-start"}))
				games[data.id].player1.socket.send(JSON.stringify({game: games[data.id], message: "game-start"}))
			}
			return
		}
		console.log(data)
		...
		games[id].player1.socket.send(JSON.stringify({game: games[id], message: "game-move"}))
		games[id].player2.socket.send(JSON.stringify({game: games[id], message: "game-move"}))
	...
	for (let i = 0; i < games.length; i++) {
		if (games[i].player2 == null) {
			...
			games[i].player2.socket.send(JSON.stringify({id: games[i].id, message: "name", playernr: 2}))
			return
		}
	}
	...
	newgame.player1.socket.send(JSON.stringify({id: newgame.id, message: "name", playernr: 1}))
